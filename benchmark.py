...
def main():
    ...
    # backward
    loss_naive = (out_naive ** 2).mean()
    grads_naive = torch.autograd.grad(loss_naive, (q, k, v, beta_logits), retain_graph=True)

    loss_ref = (out_ref ** 2).mean()
    grads_ref = torch.autograd.grad(loss_ref, (q, k, v, beta_logits))

    names = ["q", "k", "v", "beta_logits"]
    for name, gn, gr in zip(names, grads_naive, grads_ref):
